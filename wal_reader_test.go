...
func TestWALReader(t *testing.T) {
	t.Run("OK", func(t *testing.T) {
		...
		r := litestream.NewWALReader(bytes.NewReader(b))
	...
	t.Run("ZeroLength", func(t *testing.T) {
		r := litestream.NewWALReader(bytes.NewReader(nil))
	...
	t.Run("PartialHeader", func(t *testing.T) {
		r := litestream.NewWALReader(bytes.NewReader(make([]byte, 10)))
		if err := r.ReadHeader(); err != io.EOF {
			t.Fatalf("unexpected error: %#v", err)
		}
	})
	...
	t.Run("BadMagic", func(t *testing.T) {
		r := litestream.NewWALReader(bytes.NewReader(make([]byte, 32)))
	...
	t.Run("BadHeaderChecksum", func(t *testing.T) {
		data := []byte{
			...
			0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}
		r := litestream.NewWALReader(bytes.NewReader(data))
		if err := r.ReadHeader(); err != io.EOF {
			t.Fatalf("unexpected error: %#v", err)
		}
	})
	...
	t.Run("ErrPartialFrameHeader", func(t *testing.T) {
		...
		r := litestream.NewWALReader(bytes.NewReader(b[:40]))
		if err := r.ReadHeader(); err != nil {
			t.Fatal(err)
	...
	t.Run("ErrFrameHeaderOnly", func(t *testing.T) {
		...
		r := litestream.NewWALReader(bytes.NewReader(b[:56]))
		if err := r.ReadHeader(); err != nil {
			t.Fatal(err)
		} else if _, _, err := r.ReadFrame(make([]byte, 4096)); err != io.EOF {
			t.Fatalf("unexpected error: %#v", err)
	...
	t.Run("ErrPartialFrameData", func(t *testing.T) {
		...
		r := litestream.NewWALReader(bytes.NewReader(b[:1000]))
		if err := r.ReadHeader(); err != nil {
			t.Fatal(err)
		} else if _, _, err := r.ReadFrame(make([]byte, 4096)); err != io.EOF {
			t.Fatalf("unexpected error: %#v", err)
