...
const char* s3vfsFilenameDatabase(const char* zName) {
  return zName == NULL ? NULL : sqlite3_filename_database((sqlite3_filename)zName);
}

const char* s3vfsURIKey(const char* zName, int n) {
  return zName == NULL ? NULL : sqlite3_uri_key((sqlite3_filename)zName, n);
}

const char* s3vfsURIParameter(const char* zName, const char* zParam) {
  return zName == NULL ? NULL : sqlite3_uri_parameter((sqlite3_filename)zName, zParam);
}
...
const sqlite3_io_methods s3vfs_io_methods = {
  ...
  s3vfsLock,
  s3vfsUnlock,
  s3vfsCheckReservedLock,
  s3vfsFileControl,
  s3vfsSectorSize,
  s3vfsDeviceCharacteristics,
};
