...
func TestReadConfigFile_LeaseConfig(t *testing.T) {
	filename := filepath.Join(t.TempDir(), "litestream.yml")
	if err := os.WriteFile(filename, []byte(`
dbs:
  - path: /tmp/test.db
...
`[1:]), 0666); err != nil {
	...
	if len(config.DBs) != 1 {
		t.Fatalf("expected one database config")
	}
	lease := config.DBs[0].Lease
	if !lease.Required {
		t.Fatal("expected lease to be required")
	}
	if lease.TTL == nil || *lease.TTL != 45*time.Second {
		t.Fatalf("lease.TTL=%v, want 45s", lease.TTL)
	}
	if lease.Heartbeat == nil || *lease.Heartbeat != 10*time.Second {
		t.Fatalf("lease.Heartbeat=%v, want 10s", lease.Heartbeat)
	}
	if lease.AcquireTimeout == nil || *lease.AcquireTimeout != 2*time.Minute {
		t.Fatalf("lease.AcquireTimeout=%v, want 2m", lease.AcquireTimeout)
	}
}

func TestConfig_Validate_Lease(t *testing.T) {
	tests := []struct {
		name string
		yaml string
		err  error
...
dbs:
  - path: /tmp/test.db
    ...
    replica:
      url: file:///tmp/replica
`,
			err: main.ErrInvalidLeaseAcquireTimeout,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := main.ParseConfig(strings.NewReader(tt.yaml), false)
			if !errors.Is(err, tt.err) {
				t.Fatalf("ParseConfig() error=%v, want %v", err, tt.err)
			}
		})
	}
}
