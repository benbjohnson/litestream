...
func TestCalcWALSize(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			pageSize: 4096,
			pageN:    500000,
			expected: int64(litestream.WALHeaderSize) + (int64(litestream.WALFrameHeaderSize+4096) * 500000),
		},
		{
			name:     "16KB pages, 500k pages",
			pageSize: 16384,
			pageN:    500000,
			// Expected: 32 + (24+16384)*500000 = 32 + 16408*500000 = 8,204,000,032 bytes (~8.2 GB)
			expected: int64(litestream.WALHeaderSize) + (int64(litestream.WALFrameHeaderSize+16384) * 500000),
		},
		{
			name:     "32KB pages, 500k pages",
			pageSize: 32768,
			pageN:    500000,
			// Expected: 32 + (24+32768)*500000 = 32 + 32792*500000 = 16,396,000,032 bytes (~16.4 GB)
			// Bug would give: ~3.5 GB due to uint32 overflow
			expected: int64(litestream.WALHeaderSize) + (int64(litestream.WALFrameHeaderSize+32768) * 500000),
		},
		{
			name:     "64KB pages, 500k pages",
			pageSize: 65536,
			pageN:    500000,
			// Expected: 32 + (24+65536)*500000 = 32 + 65560*500000 = 32,780,000,032 bytes (~32.8 GB)
			// Bug would give: ~2.7 GB due to uint32 overflow
			expected: int64(litestream.WALHeaderSize) + (int64(litestream.WALFrameHeaderSize+65536) * 500000),
		},
		{
			name:     "1KB pages, 1k pages (min checkpoint)",
			pageSize: 1024,
			pageN:    1000,
			expected: int64(litestream.WALHeaderSize) + (int64(litestream.WALFrameHeaderSize+1024) * 1000),
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := litestream.CalcWALSizeForTest(tt.pageSize, tt.pageN)
			if got != tt.expected {
			...
			// Ensure result is positive and reasonable (not overflowed)
			if got <= 0 {
				t.Errorf("calcWALSize(%d, %d) = %d, should be positive", tt.pageSize, tt.pageN, got)
			}

			// For large page sizes, verify the result is actually large
			if tt.pageSize >= 32768 && tt.pageN >= 100000 {
				minExpected := int64(10 * 1024 * 1024 * 1024) // 10 GB minimum for large configs
				if got < minExpected {
